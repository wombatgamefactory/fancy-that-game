...
function init() {
  ...
  // A RELOAD OPENS AT THE TOP TOO (10 August). The browser's default is to
  // restore the scroll offset a reload was fired from, and it does so after the
  // page has laid out - so a player who reloads mid-game gets the setup screen's
  // or the game's old offset re-applied over whatever we render. There is one
  // screen and no history to be kind about here; the save is what carries a game
  // across a reload, not the scroll position.
